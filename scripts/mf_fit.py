...
if __name__ == '__main__':
    ...
    from src.utils import ensure_dirs, construct_sparse_adj_mat, load_data_split, load_hps_from_scratch
    import pickle
    import time
    import torch
    ...
    cli = True # Args from command line
    ...
    if cli:
        ...
        seed = 1234
        dataset_name = 'knn' # random | knn | one of the datasets
    ...
    if cli:
        ...
        train_loss_curve = [train_loss_curve[i] for i in range(0, len(train_loss_curve), 10)]
        test_loss_curve = [test_loss_curve[i] for i in range(0, len(test_loss_curve), 10)]
        gs_res = {
            'train_loss':train_loss, 'test_loss':test_loss,
            'train_loss_curve':train_loss_curve, 'val_loss_curve':test_loss_curve, 'fit_time':fit_time,
            'hp_idx':hp_idx, 'split_idx':split_idx,
            **{k:[] for k in hps.keys()}
        }
    ...
    # Save model but not if running cli gridsearch
    if not cli:
